...
Conviene dejarlos anotar los tres tiempos en la pizarra (**SIN ÍNDICE** / **CON ÍNDICE** / **ÍNDICE PROHIBIDO**) antes de explicar nada. El factor sale de dividir números que ellos midieron.
...
- **2.3** `CREATE INDEX` + `ANALYZE`: **0,6 s**. El plan pasa a `Bitmap Index Scan on idx_puntos_geom` + `Bitmap Heap Scan`, y desaparece el JIT. **~21 ms → factor ~100× medido** (si al 2.2 se le descuenta el JIT, el factor "puro" ronda 25× — sigue siendo el orden de magnitud del libreto, pero es el número honesto para la discusión). Índice **20 MB** sobre una tabla de **33 MB** (los 53 MB que reporta `pg_total_relation_size` después son tabla + índice — ojo con contarlo dos veces).
